| TYPE DISCIPLINE   | Erlang | Clojure | Haskell | Scala | Java6 | CPython3 | F#/OCaml |
|-------------------+--------+---------+---------+-------+-------+----------+----------|
| strongly typed    | yes    | yes     | yes     | yes   | yes   | yes      |          |
| statically typed  | no     | no      | yes     | yes   | yes   | no       |          |
| type inference    | n/a    | n/a     | yes     | yes   | no    | n/a      | yes      |
| type parameters   |        |         | yes     | yes   | yes   |          |          |
| abstract types    |        |         |         | yes   | no    |          |          |
| structural types  |        |         |         |       |       | yes      |          |
| existential types |        |         |         |       |       |          |          |

Notes: Python's duck typing is similar in spirit to structural types.


| OBJECTS                                                        | Erlang | Clojure | Haskell | Scala | Java6 | CPython3 | F#/OCaml |
|----------------------------------------------------------------+--------+---------+---------+-------+-------+----------+----------|
| objects (function/data containers that are first-class values) |        |         |         | yes   | yes   | yes      |          |
| all values are objects                                         |        |         |         | yes   | no    |          |          |
| interfaces (contracts) with abstract functions                 |        |         |         | yes   | yes   |          |          |
| interfaces (contracts) with abstract data                      |        |         |         | yes   | no    |          |          |
| interface inheritance                                          |        |         |         | yes   | yes   | yes      |          |
| concrete inheritance                                           |        |         |         | yes   | yes   |          |          |
| multiple interface inheritance                                 |        |         |         | yes   | yes   |          |          |
| multiple concrete inheritance                                  |        |         |         | yes   | no    |          |          |
| data hiding enforced                                           |        |         |         | yes   | yes   | no       |          |


| FUNCTIONS                         | Erlang | Clojure | Haskell | Scala | Java6 | CPython3 | F#/OCaml |
|-----------------------------------+--------+---------+---------+-------+-------+----------+----------|
| operators evaluate like functions |        |         |         | yes   | no    |          |          |
| operator overloading              |        |         | yes     | yes   | no    | yes      |          |
| first-class functions             | yes    | yes     | yes     | yes   | no    | yes      |          |
| anonymous functions               | yes    | yes     | yes     | yes   | no    | yes      |          |
| currying                          |        |         | yes     | yes   | no    |          |          |
| partial application of functions  |        |         | yes     | yes   | no    |          |          |
| full tail call optimization       | yes    | no      |         | no    |       | no       |          |
| overloading                       |        | yes     |         | yes   | yes   |          |          |
| default parameters                |        |         |         | yes   | no    | yes      |          |
| unstructured side-effects         | yes    | yes     | no      | yes   | yes   | yes      |          |

Notes: Scala's methods aren't themselves first-class but can always be treated as such. 


| BINDINGS & SCOPE  | Erlang | Clojure | Haskell | Scala | Java6 | CPython3 | F#/OCaml |
|-------------------+--------+---------+---------+-------+-------+----------+----------|
| lexical scope     |        |         |         | yes   | yes   |          |          |
| single assignment | yes    | no      | yes     | both  | both  | no       |          |
| closures          |        |         |         | yes   | no    |          |          |


| LIBRARY             | Erlang | Clojure | Haskell | Scala | Java6 | CPython3 | F#/OCaml |
|---------------------+--------+---------+---------+-------+-------+----------+----------|
| implicit functions  |        |         |         | yes   | no    |          |          |
| implicit parameters |        |         |         | yes   | no    |          |          |
| first-class modules |        |         |         |       |       | yes      |          |

Notes: Scala 2.8's package objects can only add members to a package.


| EVALUATION                            | Erlang | Clojure | Haskell | Scala | Java6 | CPython3 | F#/OCaml |
|---------------------------------------+--------+---------+---------+-------+-------+----------+----------|
| lazy evaluation                       |        |         |         |       |       |          |          |
| lazy initialization of fields         |        |         |         | yes   | no    |          |          |
| delayed parameter evaluation possible |        |         |         | yes   | no    |          |          |
| uniform access principle              |        |         |         | yes   | no    | no       |          |

Notes: Only Java's static fields are initialized lazily.


| LANGUAGE                       | Erlang | Clojure | Haskell | Scala | Java6 | CPython3 | F#/OCaml |
|--------------------------------+--------+---------+---------+-------+-------+----------+----------|
| preprocessor                   |        | yes     |         | no    | no    |          |          |
| pattern matching               | yes    | yes     | yes     | yes   | no    | no       |          |
| pattern matching in parameters | yes    |         | yes     | no    | no    | no       |          |


| CONCURRENCY             | Erlang | Clojure | Haskell | Scala | Java6 | CPython3 | F#/OCaml |
|-------------------------+--------+---------+---------+-------+-------+----------+----------|
| true parallelism        | yes    | yes     | yes     | yes   | yes   | no       | yes      |
| unmanaged mutable state |        | no      | no      | yes   | yes   | yes      |          |

Notes: CPython's global interpreter lock prevents to threads from making progress at the same time.
